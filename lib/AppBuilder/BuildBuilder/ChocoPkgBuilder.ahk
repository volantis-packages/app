class ChocoPkgBuilder extends BuildBuilderBase {
    ...
    Build(version) {
        ...
        if (!FileExist(installer)) {
            throw AppBuildException("Installer file doesn't exist, cannot build chocolatey package.")
        }

        hash := this.app["FileHasher"].Hash(installer, FileHasher.HASH_TYPE_SHA256)

        if (!hash) {
            throw AppBuildException("Failed to create an SHA256 hash of the installer file.")
        }
