class GitHubBuildDeployer extends BuildDeployerBase {
    Deploy(deployInfo) {
        ...
        responseData := request.Post(this.GetPostData(deployInfo))
        success := !!(request.GetStatusCode() == 200)

        if (!success || !responseData) {
            throw AppBuildException("Failed to post release to GitHub")
        }
        ...
        if (!responseObj.Has("upload_url") || !responseObj["upload_url"]) {
            throw AppBuildException("Could not determine release file upload URL")
        }

        success := this.UploadInstaller(deployInfo, responseObj["upload_url"])

        if (!success) {
            throw AppBuildException("Failed to upload installer file")
        }
